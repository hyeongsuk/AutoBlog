"""
AI 콘텐츠 생성 모듈 (Claude API 버전)
Anthropic Claude API를 활용한 블로그 포스트 자동 생성
"""
...
class ClaudeContentGenerator:
    def __init__(self):
        """Claude AI 콘텐츠 생성기 초기화"""
        self.api_key = os.getenv('CLAUDE_API_KEY')

        if not self.api_key:
            raise ValueError("환경 변수에 CLAUDE_API_KEY를 설정해주세요.")

        # Claude API 설정
        self.client = Anthropic(api_key=self.api_key)
        self.model = "claude-3-5-sonnet-20241022"  # 최신 Claude 3.5 Sonnet
    ...
    def generate_blog_post(self, keyword, tone="친근하고 정보적인", length="중간"):
        ...
        # 프롬프트 생성
        prompt = f"""당신은 전문 블로그 작가입니다. 다음 키워드에 대한 네이버 블로그 포스트를 작성해주세요.
...
- 실용적이고 도움이 되는 정보 제공"""
        ...
        try:
            ...
            print(f"Claude AI 콘텐츠 생성 중 오류 발생: {str(e)}")
            # 오류 발생 시 기본 콘텐츠 반환
            return {
